...
export class ProductDetailComponent implements OnInit {
  ...
  private loadProduct(id: string): void {
    this.api.get<Product>(`/products/${id}`).subscribe({
      ...
      error: () => { this.toast.error('Failed to load product'); this.loading.set(false); },
    });
  }
  ...
  toggleWishlist(p: Product): void {
    this.wishlistToggling.set(true);
    if (this.isWishlisted()) {
      this.api.delete<any>('/wishlist/' + p.id).subscribe({
        next: (res) => {
          ...
          this.wishlistToggling.set(false);
        },
        error: () => { this.toast.error('Failed to remove from wishlist'); this.wishlistToggling.set(false); },
      });
    } else {
      this.api.post<any>('/wishlist/' + p.id, {}).subscribe({
        ...
        error: () => { this.toast.error('Failed to add to wishlist'); this.wishlistToggling.set(false); },
      });
    }
  }
